...
async def start_streaming():
    ...
    peer_connection = RTCPeerConnection(
        configuration=RTCConfiguration(
            iceServers=[
                RTCIceServer(**url_data.dict())
                for url_data in sturn_turn_config
            ]
        )
    )
    peer_connection.addTrack(get_media_player())
    ...
    async def on_connectionstatechange():
        if (
            ...
            print(
                peer_connection.connectionState, peer_connection.iceConnectionState
            )
